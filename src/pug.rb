...
class Pug
  ...
  match /whattimeisit/i, method: :command_whattimeisit
  ...
  match /crestrict ([\S]+) (.+)/i, method: :admin_restrictcaptain
  match /cauthorize ([\S]+)/i, method: :admin_authorize_captain 
  match /mrestrict ([\S]+) (.+)/i, method: :admin_restrictmedic
  match /mauthorize ([\S]+)/i, method: :admin_authorize_medic  
  match /restrictions/i, method: :admin_restrictions
  match /crestrictions/i, method: :admin_crestrictions
  match /mrestrictions/i, method: :admin_mrestrictions
  ...
  # !man
  def command_help m
    message "Player related commands: !add, !remove, !list, !sub, !need, !afk, !stats, !nick"
  ...
  def command_whattimeisit m
    message "#{ colourize "**HAMMERTIME**", const["colours"]["red"]}"
  end
  ...
  # !crestrict
  def admin_restrictcaptain m, nick, duration
    return unless require_admin m

    restrict_captain m.user, nick, duration
  end
  
  # !mrestrict
  def admin_restrictmedic m, nick, duration
    return unless require_admin m

    restrict_medic m.user, nick, duration
  end
  ...
  # !cauthorize
  def admin_authorize_captain m, nick
    return unless require_admin m

    authorize_captain m.user, nick
  end
  
  # !mauthorize
  def admin_authorize_medic m, nick
    return unless require_admin m

    authorize_medic m.user, nick
  end
 ...
 # !restrictions
  def admin_restrictions m
   restrictions = Restriction.where("name = ? AND duration != NULL")
 # restrictions = Restriction.find(:conditions => "name = ? AND duration != NULL") 
   return unless require_admin m
   return notice m.user, "Players restricted: "
   return notice m.user, restrictions
  end

 # !crestrictions
  def admin_crestrictions m
    crestrictions = Restrictcaptain.where("name = ? AND duration != NULL")
  # crestrictions = Restrictcaptain.find(:conditions => "name = ? AND duration != NULL")
    return notice m.user, "Captains restricted: "
    return notice m.user, crestrictions
  end

 # !mrestrictions
  def admin_mrestrictions m
    mrestrictions = Restrictmedic.where("name = ? AND duration != NULL")
  # mrestrictions = Restrictmedic.find(:conditions => "name = ? AND duration != NULL")
    return notice m.user, "Medics restricted: "
    return notice m.user, mrestrictions
  end
